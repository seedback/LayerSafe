...
def generate_cutout(
  ...
  with BuildPart() as normal_base:
    with BuildSketch():
      c = Circle(base_diameter/2 + tolerance/2, align=(Align.MIN, Align.MIN))
  ...
  with BuildPart() as flattener:
    with Locations((base_diameter/2 , base_diameter/2 + tolerance/2, 0)):
      Box(base_diameter+tolerance*2, base_diameter+tolerance*2, 1, align=(Align.CENTER, Align.CENTER, Align.MAX))
  normal_base.part -= flattener.part
  
  return_compound = Compound([normal_base.part]).translate((-tolerance/2,0,0))
...
if __name__ == "__main__":
  cutout, flattener = generate_cutout(32)

  show( cutout, flattener)
#%%